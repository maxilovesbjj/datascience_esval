...
- explorar y depurar datos de sensores de **caudal** y **presión manométrica**,
- derivar variables hidráulicas relevantes (velocidad, presión absoluta, screening de cavitación),
- detectar y caracterizar **eventos operacionales** (aperturas/cierres; bruscos/suaves; cortos/largos),
- identificar **anomalías** (Isolation Forest),
- entrenar modelos supervisados para **detección** y **predicción probabilística** de eventos bruscos.

> **Importante:** el repositorio no incluye datos crudos por confidencialidad. El pipeline asume parquets procesados en `data/processed/`, generados desde el Excel original.

---

## Contexto del problema

Se dispone de un registro temporal de presión y caudal en una tubería que sufrió una falla. El objetivo es **extraer evidencia desde los datos** (sin asumir causa a priori), conectando patrones con física de transientes y cuantificando si el sistema exhibe:
...
- factibilidad de detectar/anticipar eventos bruscos mediante ML.

---

## Datos y variables

### Sensores (serie a ~1 minuto)
- Caudal: $Q(t)$ [L/s]
...
$$
D = 315\,\mathrm{mm} = 0.315\,\mathrm{m}
$$
...
Velocidad media (con conversión de unidades aplicada en notebooks):
...
Como el sensor entrega presión manométrica:

$$
P_{abs}(t)\approx P_g(t)+P_{atm}
$$

donde $P_{atm}$ puede ajustarse por altitud (en notebooks se usa un valor por defecto, modificable).
...
Indicador hidráulico (número de cavitación):
...
- Verificación del muestreo (~1 minuto) y primeras estadísticas.
...
- apertura (ventana $\pm 1$ min),
- apertura brusca (ventana $\pm 1$ min),
- cierre (start),
- cierre brusco (start).
...
$$
\mathbb{P}\left(\text{inicio brusco en }(t,t+K]\right),\qquad K\in\{10,30,60\}\ \text{min}
$$

**Restricción clave (evitar leakage):** los features en $t$ usan solo historia $\le t$ (rolling hacia atrás). Los labels pueden mirar futuro, pero no “contaminan” features.

**Modelos usados**
- Regresión Logística (LogReg): interpretable (coeficientes), con Imputer(median) + RobustScaler.
- HistGradientBoosting (HGB): no lineal, con Imputer(median).

**Métricas**
- Por minuto: PR-AUC (Average Precision) y prevalencia.
- Por evento (operacional):
  - Detección: hit si existe alarma en $[t_0-1,\,t_0+1]$.
  - Forecast: hit si existe alarma en $[t_0-K,\,t_0-1]$, con lead time.
...
**Conclusión (screening):** cavitación por “baja de presión bajo vapor” no aparece en estos datos 1-min.  
**Nota:** con muestreo 1-min pueden no capturarse mínimos instantáneos de transientes rápidos; por eso se reporta como screening.
...
Se observa asimetría operacional (familias cuantificadas en notebooks).
...
- En forecast, el desempeño crece con el horizonte $K$ (problema menos “instantáneo”).
- La selección final de modelo/umbral depende del objetivo operacional (minimizar falsas alarmas vs maximizar recall por evento).

---

## Limitaciones y alcance
- La serie está muestreada a ~1 minuto: transientes de segundos (golpe de ariete clásico) pueden quedar submuestreados.
- Las conclusiones hidráulicas se plantean como evidencia macroscópica (cambios de régimen, escalones/rampas), screening de cavitación y cuantificación de repetitividad/ciclos a escala minuto.
